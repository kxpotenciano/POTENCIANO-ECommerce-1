...
if(
	...
	if(count($errorCollector) == 0){
		...
		$suStatement->close();
		$userid = $userRow['id'];
		$ipStatement = $con->prepare("INSERT INTO profile(firstname,middlename,lastname,suffix,user_id) VALUES (?,?,?,?,?)");
		$ipStatement->bind_param("ssssi",$firstname,$middlename,$lastname,$suffix,$userid);
		print_r($con->errors);
		$ipStatement->execute();
		$ipStatement->close();
